...
from roco.engine.artifacts.domains.hit_count.matchers import sum_hit_count_delta_amount
from roco.engine.artifacts.domains.status_mark.freeze import is_freeze_status
from roco.engine.artifacts.pak_ref_common import BUFF_BASE_IDS, BUFFBASE_ORDER, _all_skill_cost_reduce_amount, _all_zero, _as_int_tuple, _base_rows, _element_mask, _is_burn_status, _is_poison_status, _op, _pack_buff_delta_from_buff_ids, _param, _param_int, _skill_dam_type_to_element, buff_type, effect_type, EFFECT_ORDER, EFFECT_PARAMS
from roco.engine.artifacts.domains.active_buff.after_attack import link_after_attack_buff_install
from roco.engine.kernel.core.rows import TARGET_ENEMY, TIMING_PAK_BEFORE_HURT
...
def _link_after_skill_element_child_buff(buff_id: int, timing: int, target: int, rate: int, *, source_name: str) -> tuple[LinkedOp, ...] | None:
    ...
    if target_code == 0 and _all_zero((tail,)):
        ...
        hit_count = sum_hit_count_delta_amount(refs)
        if hit_count > 0:
            return tuple(
    ...
    if target_code == 2 and _all_zero((tail,)):
        ...
        if refs and all(is_freeze_status(ref_id) for ref_id in refs):
            return tuple(
