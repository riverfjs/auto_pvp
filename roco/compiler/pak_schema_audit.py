...
labels every hand-written rule (``exact_effects.jsonl`` /
``prefix_handlers.jsonl``) against those axes.  The output is purely
...
the current rule debt is structurally addressable in pak and therefore
collapsible into family decoders in future phases.
...
Run::
    ...
    uv run python -m roco.compiler.pak_schema_audit         # write
    uv run python -m roco.compiler.pak_schema_audit --check # CI gate
...
# Repo root: roco/compiler/pak_schema_audit.py -> parents[2]
...
def _load_exact_rules() -> list[dict]:
    ...
    if not EXACT_JSONL.exists():
        return out
    with EXACT_JSONL.open("r", encoding="utf-8") as fp:
        for line in fp:
            line = line.strip()
...
def _load_prefix_rules() -> tuple[dict[int, dict], list[dict]]:
    """Return (prefix_rules, base_id_overrides).

    Two row shapes coexist in prefix_handlers.jsonl: ``{"prefix": ..., "handler": ...}``
    and ``{"base_id": ..., "handler": ...}``.  Only the prefix rows are
    addressed by buffbase_order rekeying — the base_id overrides are
    exact-match anchors that stay regardless of axis choice.
    """
    by_prefix: dict[int, dict] = {}
    overrides: list[dict] = []
    if not PREFIX_JSONL.exists():
        return by_prefix, overrides
    with PREFIX_JSONL.open("r", encoding="utf-8") as fp:
        for line in fp:
            line = line.strip()
...
def render_markdown(
    ...
    lines.append("")
    lines.append(
        "_Auto-generated by `roco/compiler/pak_schema_audit.py`. "
        "Do not edit by hand. Re-run with "
        "`uv run python -m roco.compiler.pak_schema_audit`._"
    )
    lines.append("")
    lines.append(
    ...
    lines.append(
        ...
        "`coverage` is sourced from `roco/compiler/rules/effect_families.jsonl` "
    ...
    lines.append("### 4a. `exact_effects.jsonl` clusters")
    lines.append("")
    ...
    lines.append("### 4b. `prefix_handlers.jsonl` rekey candidates")
    lines.append("")
    lines.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.check:
        ...
        if on_disk != fresh:
            sys.stderr.write(
                f"stale: {AUDIT_MD}\n"
                "re-run: uv run python -m roco.compiler.pak_schema_audit\n"
            )
            return 1
        return 0
