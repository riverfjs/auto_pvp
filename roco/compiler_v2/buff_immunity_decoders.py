"""Derive BUFF_CONF-direct immunity flags from pak descriptions.
...
Downstream consumers import :data:`IMMUNITY_SPECS` from here.  The
loader scans pak ``BUFF_CONF.desc`` by default and returns
``dict[int, int]`` (buff_id → packed flags); it does not touch the
kernel or the runtime decoder pipeline.
...
def load_buff_immunity_table(
    ...
    """Load buff-immunity semantics into ``buff_id → packed_flags``.
    ...
    * ``evidence`` must start with ``BUFF_CONF.json[<buff_id>].desc=``.
    ...
    for line_no, rec in _iter_rule_records(rules_path, conf):
        buff_id = int(rec["buff_id"])
        if buff_id in first_seen_line:
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: duplicate buff_id "
        ...
        if buff_id not in conf:
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: buff_id {buff_id} "
                f"not in BUFF_CONF"
            )
        pak_desc = str(rec.get("pak_desc", ""))
        if len(pak_desc) > 256:
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: pak_desc longer than "
                f"256 chars"
            )
        real_desc = str(conf[buff_id].get("desc", ""))
        if pak_desc != real_desc:
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: pak_desc {pak_desc!r} "
                f"does not match BUFF_CONF[{buff_id}].desc {real_desc!r}"
            )
        tags = rec.get("immunities") or []
        if not isinstance(tags, list) or not tags:
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: ``immunities`` must "
        ...
        for tag in tags:
            if tag not in tag_specs:
                raise RuntimeError(
                    f"buff immunity fixture line {line_no}: unknown immunity "
                    f"tag {tag!r}; allowed: {sorted(tag_specs)}"
                )
            spec = tag_specs[tag]
            if spec.keyword not in pak_desc:
                raise RuntimeError(
                    f"buff immunity fixture line {line_no}: immunity tag "
        ...
        if not evidence.startswith(expected_prefix):
            raise RuntimeError(
                f"buff immunity fixture line {line_no}: evidence must start "
...
def _derive_tags_from_desc(desc: str) -> list[str]:
    text = desc.strip()
    if not text.startswith("免疫"):
        return []
    return [spec.tag for spec in IMMUNITY_SPECS if spec.keyword in text]
...
def _iter_rule_records(
    ...
    if rules_path is None:
        ...
        for buff_id, rec in sorted(buff_conf.items()):
            ...
            tags = _derive_tags_from_desc(desc)
            if not tags:
                continue
            out.append((
                len(out) + 1,
                {
                    "buff_id": buff_id,
                    "pak_editor_name": str(rec.get("editor_name", "")),
                    "pak_desc": desc,
                    "immunities": tags,
                    "evidence": f"BUFF_CONF.json[{buff_id}].desc={desc!r}",
                },
            ))
        return out
    out: list[tuple[int, dict]] = []
