...
from roco.engine.kernel.core.catalog import ELEMENT_BUG, PET_ABILITY, PET_PRIMARY, PET_SECONDARY, SKILL_ELEMENT, STAT_HP
from roco.engine.kernel.core.ctx import StageCtx
from roco.engine.kernel.core.rows import TIMING_HOOK_ENEMY_SWITCH, TIMING_PAK_SDT, TIMING_HOOK_SWITCH_OUT
from roco.engine.kernel.core.dispatch import run_skill_timing
from roco.engine.kernel.model.state import KernelState, SideState, replace_pet, replace_side, side
...
def apply_switch_in_ability(side_state: SideState, slot: int, enemy_side: SideState | None = None) -> SideState:
    ...
    if ctx.self_global_power_delta:
        pet = pet._replace(global_power_bonus=max(-255, min(255, pet.global_power_bonus + ctx.self_global_power_delta)))
