...
from roco.engine.artifacts.domains.ability_effects.matchers import link_ability_effect_buff
from roco.engine.artifacts.domains.active_buff.immunity import link_active_immunity_buff
from roco.engine.artifacts.domains.active_buff.zero_energy import link_zero_energy_auto_switch_buff
from roco.engine.artifacts.domains.hit_count.matchers import link_conditional_hit_count_buff, link_hit_count_delta_buff, link_team_skill_hit_count_buff
from roco.engine.artifacts.domains.resources.matchers import energy_amount_from_effect_refs, link_life_drain_buff
from roco.engine.artifacts.domains.status_mark.basic import link_status_or_mark_buff
from roco.engine.artifacts.domains.status_mark.cute import link_cute_stack_buff
from roco.engine.artifacts.domains.status_mark.cute_conditions import link_cute_bench_cost_reduce_buff
from roco.engine.artifacts.domains.status_mark.freeze import link_freeze_buff
...
from roco.engine.artifacts.pak_ref_buff_modifiers import _link_after_attack_response_buff, _link_after_attack_status_buff, _link_after_skill_element_child_buff, _link_attack_cost_delta_buff, _link_damage_reduction_buff, _link_element_cost_reduce_buff, _link_first_strike_power_buff, _link_force_switch_buff, _link_global_cost_delta_buff, _link_global_power_delta_buff, _link_heal_reversal_buff, _link_power_dynamic_buff, _link_specific_skill_power_bonus_buff, _link_transmission_buff
...
def _link_entry_energy_buff(buff_id: int, target: int, rate: int, *, source_name: str) -> LinkedOp | None:
    ...
    amount = energy_amount_from_effect_refs(_as_int_tuple(base_params[4]), source_name=source_name)
...
def link_buff_ref(buff_id: int, timing: int, target: int, rate: int, p0: int, p1: int, p2: int, p3: int, *, source_name: str, link_ref_id) -> tuple[LinkedBuff, ...]:
    ...
    if after_skill is not None:
        return after_skill
    linked = link_ability_effect_buff(buff_id, timing, target, rate, source_name=source_name) or link_status_or_mark_buff(buff_id, timing, target, rate, stack_count) or link_cute_stack_buff(buff_id, timing, target, rate) or link_freeze_buff(buff_id, timing, target, rate) or link_zero_energy_auto_switch_buff(buff_id, timing, target, rate) or link_active_immunity_buff(buff_id, timing, target, rate, source_name=source_name) or link_team_skill_hit_count_buff(buff_id, timing, target, rate, stack_count, source_name=source_name) or link_hit_count_delta_buff(buff_id, timing, target, rate, stack_count, source_name=source_name) or _link_heal_reversal_buff(buff_id, timing, target, rate, source_name=source_name) or link_life_drain_buff(buff_id, timing, target, rate) or link_cute_bench_cost_reduce_buff(buff_id, timing, target, rate, source_name=source_name) or link_conditional_hit_count_buff(buff_id, timing, target, rate, p0, source_name=source_name) or _link_transmission_buff(buff_id, timing, target, rate, p0, p1, p2, p3) or _link_entry_energy_buff(buff_id, target, rate, source_name=source_name) or _link_global_cost_delta_buff(buff_id, timing, target, rate) or _link_attack_cost_delta_buff(buff_id, timing, target, rate) or _link_element_cost_reduce_buff(buff_id, timing, target, rate) or _link_global_power_delta_buff(buff_id, timing, target, rate) or _link_after_attack_status_buff(buff_id, timing, target, rate) or _link_after_attack_response_buff(buff_id, timing, target, rate, source_name=source_name, link_ref_id=link_ref_id) or _link_damage_reduction_buff(buff_id, timing, target, rate) or _link_power_dynamic_buff(buff_id, timing, target, rate) or _link_first_strike_power_buff(buff_id, timing, target, rate) or _link_specific_skill_power_bonus_buff(buff_id, timing, target, rate) or _link_force_switch_buff(buff_id, timing, target, rate) or _link_generic_buff_delta(buff_id, timing, target, rate)
