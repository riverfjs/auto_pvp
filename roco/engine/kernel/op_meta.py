...
def handles_base_name(_entries: Sequence[tuple[str, str]]) -> Callable[[F], F]:
    ...
    Production engine ops must not use this.  ``compiler_v2`` rejects
    collected ``base_name`` declarations because current pak exports no
    longer provide reliable ``BUFFBASE_CONF.editor_name`` data.
    """
    def _identity(func: F) -> F:
        return func
    return _identity
