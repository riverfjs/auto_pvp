...
from roco.common.packing import BUFF_ATK_MAG, BUFF_ATK_PHYS, MarkIdx, _set_mark, _unpack_buff, _unpack_mark
...
def test_compound_bft_cast_skill_after_attack_links_to_active_buff_install():
    row = (buff_ref_key(20190360), pak_cast_moment_key(7), 1, 10000, 0, 0, 0, 0)
    linked = link_primitive_row(row, source_name="应对！冥想")
    assert linked.op_name == "op_apply_active_buff"
    assert linked.runtime_args() == (20190360, 2, 1, 99)
...
def test_compound_after_attack_active_response_purifies_both_sides_marks():
    assert after_attack_response_supported(20190360)
    state = make_state((1, 2, 3), (4, 5, 6))
    state = _seed_active_response(state, SIDE_B, 0, 20190360)
    state = replace_side(
        state,
        SIDE_A,
        side(state, SIDE_A)._replace(marks=_set_mark(0, MarkIdx.METEOR, 2)),
    )
    state = replace_side(
        state,
        SIDE_B,
        side(state, SIDE_B)._replace(marks=_set_mark(0, MarkIdx.THORN, 3)),
    )
    new_state = trigger_after_attack_active_buffs(
        state,
        SIDE_A,
    ...
    assert side(new_state, SIDE_A).marks == 0
    assert side(new_state, SIDE_B).marks == 0


def test_compound_after_attack_active_response_does_not_fire_without_damage():
    assert after_attack_response_supported(20190360)
    state = make_state((1, 2, 3), (4, 5, 6))
    state = _seed_active_response(state, SIDE_B, 0, 20190360)
    marks = _set_mark(0, MarkIdx.METEOR, 2)
    state = replace_side(state, SIDE_A, side(state, SIDE_A)._replace(marks=marks))
    new_state = trigger_after_attack_active_buffs(
        state,
        SIDE_A,
        0,
    ...
    assert side(new_state, SIDE_A).marks == marks
