...
(regenerated from BUFF_CONF) and the prefix → kernel-handler seed lives
in :file:`roco/compiler/rules/prefix_handlers.jsonl`.  This module now
only carries the small ``Timing`` enum (1:1 with pak ``cast_moment``)
and the compiled dataclasses used by the data layer.
